...
class GroupManagementWidget(QWidget):
    data_changed = Signal()  # sygnał informujący o zmianie danych
    ...
    def _odswiez_liste_grup(self):
        ...
        for i, grupa in enumerate(self.zarzadca.grupy):
            item = QListWidgetItem(grupa.nazwa)
            item.setData(256, i)  # przechowujemy indeks
            self.lista_grup.addItem(item)

    def _wybrano_grupe(self, row: int):
        if row >= 0:
            indeks = self.lista_grup.item(row).data(256)
            self.aktualna_grupa = self.zarzadca.grupy[indeks]
            self._pokaz_metody()
        else:
            self.aktualna_grupa = None
    ...
    def _pokaz_metody(self):
        ...
        for i, metoda in enumerate(grupa.metody):
            # Dla każdej metody wyświetlamy wiersze dla każdego przedziału
            for j, przedzial in enumerate(self.zarzadca.przedzialy):
                ...
                # Lp. (tylko dla pierwszego wiersza metody)
                if j == 0:
                ...
                # Przycisk edycji tylko dla pierwszego wiersza metody
                if j == 0:
    ...
    def _usun_grupe(self):
        ...
        if row < 0:
            ...
            return
        indeks = self.lista_grup.item(row).data(256)
        grupa = self.zarzadca.grupy[indeks]
    ...
    def _edytuj_grupe(self):
        ...
        if row < 0:
            ...
            return
        indeks = self.lista_grup.item(row).data(256)
        stara_nazwa = self.zarzadca.grupy[indeks].nazwa
